...
def correct_perspective(image):
    ...
    # Find corners (intersections of horizontal and vertical lines)
    corners = []
    for h_line in horizontal_lines[:2]:  # Use first two horizontal lines
        for v_line in vertical_lines[:2]:  # Use first two vertical lines
            corner = line_intersection(h_line, v_line)
            corners.append(corner)
    
    # Order corners: top-left, top-right, bottom-right, bottom-left
    corners = sorted(corners, key=lambda x: x[0])  # Sort by x
...
def detect_and_draw_circles(binary_image, gray_image, min_radius=50, max_radius=180, param1 =50, param2 =28):
    ...
    if circles is not None:
        ...
        try:
            grid_start_x, grid_start_y, cell_size = calculate_grid(x_coords, y_coords, width, height)
...
# # Save images to PDF
# pdf_output_path = os.path.join(path, "CorrectPerspective.pdf")
# print("Saving Images")
