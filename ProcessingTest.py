...
def detect_and_draw_circles(binary_image, gray_image, noClusters, min_radius=50, max_radius=200, param1=50, param2=28):
    """Detect circles in the image and draw grid, yellow areas, and counts."""
    print("I AM FINDING CIRCLES")
    ...
    if noClusters:
        ...
        x_coords, y_coords, marked_image = findBlobs(binary_image, 150, 2000000000)
    elif circles is not None:
        circles = np.round(circles[0, :]).astype(int)
        x_coords = circles[:, 0]
        y_coords = circles[:, 1]
        marked_image = cv2.cvtColor(binary_image, cv2.COLOR_GRAY2BGR)
        cv2.imshow("markedImage", resize_for_display(marked_image))

        try:
            grid_start_x, grid_start_y, cell_size, slant_angle = calculate_grid(x_coords, y_coords, width, height, binary_image, gray_image, debug=True)
            grid_calculated = True
        except ValueError as e:
            print(f"Error in grid calculation: {e}")
            print("Attempting blob detection with looser parameters")
            x_coords, y_coords, marked_image = findBlobs(binary_image, 100, 25000000)  # Looser parameters
...
def calculate_grid(x_coords, y_coords, width, height, binarized_image, gray_image, debug=True):
    ...
    def find_clusters(coords, min_count=2):
        ...
        print("median " + str(median_diff))
        ######make this change, good change 
