...
def plot_logarithmic_graph(data, log_base=10):
    normValue = (data[0]['y'][0] + data[1]['y'][0]) / 2
    y_data = [normalize_array(d['y'], normValue) for d in data]
    
    colors = [ATCcolours[i] if d['is_atc'] else nonATCcolours[i] for i, d in enumerate(data)]
    markers = ['o', 's', '^', 'D']
    labels = [d['key'] for d in data]
    atc_labels = ['+ATC' if d['is_atc'] else '-ATC' for d in data]
    ...
    plt.ylim(0, 120)
    plt.xlabel(f'Dilution Series (log base {log_base})', fontsize=16, fontweight='bold')
    plt.ylabel('Relative Growth (%)', fontsize=16, fontweight='bold')
    plt.legend(lines, [l.get_label() for l in lines], fontsize=14, loc='upper right', bbox_to_anchor=(0.98, 0.98), ncol=1, frameon=True, facecolor='white', edgecolor='none', framealpha=0.7)
    plt.tick_params(axis='both', which='major', labelsize=14)
    plt.tight_layout()
    
    atc_series = [d for d in data if d['is_atc']]
    non_atc_series = [d for d in data if not d['is_atc']]
    
    atc_averages = [sum([s['y'][i] for s in atc_series]) / len(atc_series) for i in range(len(DILUTIONSERIES))]
    non_atc_averages = [sum([s['y'][i] for s in non_atc_series]) / len(non_atc_series) for i in range(len(DILUTIONSERIES))]
    ...
    atc_line = sns.scatterplot(x=DILUTIONSERIES, y=atc_averages, color=ATCcolours[0], marker='o', label='+ATC Average', s=80)
    non_atc_line = sns.scatterplot(x=DILUTIONSERIES, y=non_atc_averages, color=nonATCcolours[0], marker='s', label='-ATC Average', s=80)
    
    plt.title("ATC vs Non-ATC Average Growth Curve", fontsize=20, fontweight='bold', pad=20)
    plt.ylim(0, 120)
    plt.xlabel(f'Dilution Series (log base {log_base})', fontsize=16, fontweight='bold')
    plt.ylabel('Relative Growth (%)', fontsize=16, fontweight='bold')
    plt.legend(fontsize=14, loc='upper right', bbox_to_anchor=(0.98, 0.98), ncol=1, frameon=True, facecolor='white', edgecolor='none', framealpha=0.7)
    plt.tick_params(axis='both', which='major', labelsize=14)
    plt.tight_layout()
    plt.show()
...
data = [
    {'y': [100, 88.8, 79.5, 74.2, 68.2, 65.4, 59.4], 'key': 'Jan', 'is_atc': True},
    {'y': [100, 89.2, 80.6, 72.1, 65.3, 62.3, 55.7], 'key': 'Feb', 'is_atc': True},
    {'y': [100, 92.1, 84.2, 77.5, 71.8, 68.2, 62.4], 'key': 'Mar', 'is_atc': False},
    {'y': [100, 90.3, 82.1, 75.2, 69.6, 65.1, 60.2], 'key': 'Apr', 'is_atc': False}
]

DILUTIONSERIES = [0, 1, 2, 3, 4, 5, 6]
fig, stats = plot_logarithmic_graph(data, log_base=2)
